        ...
        if self._input_file is None:
            try:
                ...
                if e.strerror == 'Permission denied':
                    # PATCHED for echowrite: this property is accessed from a
                    # daemon reader thread, so exit() only ends that thread
                    # (the device is simply skipped). Don't print the
                    # misleading "you must be sudo" message — sudo is not the
                    # fix here, and non-keyboard event devices (touchpads,
...
def ensure_root():
    # PATCHED for echowrite: upstream required root (os.geteuid() == 0) to use
    # this library on Linux. Our installer grants the current user access to
    # /dev/uinput and /dev/input/event* via the `input` group and a udev rule,
    # so the root check is unnecessary. See vendor/keyboard/README.md.
    pass
