...
def _populate_minimal_keymap():
    """PATCHED for echowrite: minimal fallback keycode→name mapping.

    Used when `dumpkeys` is unavailable (e.g. when the process has no
    controlling terminal, like in some SSH/container sessions). Without
    this, the library reports every key event as name='unknown', which
    breaks any app that pattern-matches on event names (like echowrite
    matching `event.name in ['ctrl', 'left ctrl', 'right ctrl']`).

    Covers only the keys echowrite and similar global-hotkey tools
    actually need: modifiers (ctrl/alt/shift/super/menu), escape, the
    F-keys used for paste-mode switching, and a few common letter keys
    for the rare case of named-letter hotkeys.
    """
    # Modifiers — the library's pressed_modifiers tracking depends on these
    # names being in all_modifiers (see _canonical_names.all_modifiers).
    register_key((29,  ()), 'ctrl')
    register_key((97,  ()), 'right ctrl')
    register_key((56,  ()), 'alt')
    register_key((100, ()), 'right alt')
    register_key((42,  ()), 'shift')
    register_key((54,  ()), 'right shift')
    register_key((125, ()), 'left windows')
    register_key((126, ()), 'right windows')
    register_key((127, ()), 'menu')

    # Escape — used by echowrite to cancel a recording.
    register_key((1,   ()), 'escape')

    # F-keys — echowrite uses F1/F2/F3 for paste mode switching.
    for code, name in [(59,'f1'),(60,'f2'),(61,'f3'),(62,'f4'),
                       (63,'f5'),(64,'f6'),(65,'f7'),(66,'f8'),
                       (67,'f9'),(68,'f10'),(87,'f11'),(88,'f12')]:
        register_key((code, ()), name)
...
def build_tables():
    ...
    # PATCHED for echowrite: dumpkeys needs a console file descriptor. When
    # the process has no controlling TTY (e.g. certain SSH/container
    # sessions), it returns non-zero and aborts init() entirely. Wrap it
    # and fall back to a hardcoded minimal mapping so the library still
    # has the keys echowrite cares about.
    dump = ''
    try:
        dump = check_output(['dumpkeys', '--keys-only'], universal_newlines=True, stderr=DEVNULL)
    except (CalledProcessError, OSError, FileNotFoundError):
        pass

    if not dump:
        _populate_minimal_keymap()
    else:
    ...
    # PATCHED: same as above — if the first dumpkeys failed, the second
    # would too. Skip the synonyms pass silently in that case.
    synonyms_template = r'^(\S+)\s+for (.+)$'
    try:
        dump = check_output(['dumpkeys', '--long-info'], universal_newlines=True, stderr=DEVNULL)
